...
export default function AdminDashboard() {
    ...
    const [alunosRecuperacao, setAlunosRecuperacao] = useState([]);
    const [carregandoDados, setCarregandoDados] = useState(true);

    // 1. Proteção de Rota Robusta
    useEffect(() => {
    ...
    // 2. Função de Cobrança WhatsApp
    const cobrarAluno = (aluno: any) => {
        ...
        const msg = encodeURIComponent(
            `Olá, ${aluno.nome}! Notamos que seu plano na academia está pendente. Vamos regularizar para você não perder o ritmo dos treinos?`
        );
    ...
    const carregarDados = useCallback(async () => {
        try {
            ...
            if (alunosRes.data) {
                const total = alunosRes.data.length;
                
                // Normalização de status para evitar erro de Case (Ativo vs ativo)
                const ativos = alunosRes.data.filter(a => 
                    String(a.status_assinatura || '').toLowerCase().trim() === 'ativo'
                ).length;

                const novosMes = alunosRes.data.filter(a => 
                ...
                // Lista para Recuperação (últimos 4 inadimplentes)
                setAlunosRecuperacao(alunosRes.data
                    .filter(a => String(a.status_assinatura || '').toLowerCase().trim() !== 'ativo')
                    .slice(0, 4));
            }

            // Gerar dados do gráfico de 12h
            const esqueleto = [];
            for (let i = 12; i >= 0; i--) {
                const d = new Date();
        ...
        <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-8 pb-32 animate-in fade-in duration-700">
            ...
            <section className="bg-zinc-900/40 border border-zinc-800/50 p-6 rounded-[2.5rem] mb-8 backdrop-blur-sm">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-zinc-400">Fluxo de Presença (12h)</h2>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <MetricCard title="Receita (Ativos)" value={`R$ ${(stats.ativos * 129).toLocaleString()}`} icon={<DollarSign size={20} />} color="border-[#9ECD1D]/20" highlight />
            ...
            <section className="bg-zinc-900/40 border border-zinc-800/50 rounded-[2.5rem] overflow-hidden backdrop-blur-sm">
                <div className="p-6 border-b border-zinc-800/50 flex justify-between items-center">
                    <h3 className="text-sm font-bold uppercase tracking-widest">Recuperação de Alunos</h3>
                    <span className="text-[10px] bg-red-500/10 text-red-500 px-3 py-1 rounded-full font-bold">AÇÃO RECOMENDADA</span>
                </div>
                                ...
                                <tr key={i} className="hover:bg-zinc-800/10 transition-colors">
                                    ...
                                    <td className="px-8 py-5 text-right">
                                        <button 
                                            ...
                                            className="inline-flex items-center gap-2 text-[#9ECD1D] text-xs font-black uppercase italic hover:scale-105 transition-transform"
                                        >
...
function MetricCard({ title, value, icon, color, highlight }: any) {
    return (
        <div className={`bg-zinc-900/40 border ${color} p-6 rounded-[2rem] flex items-center justify-between transition-all hover:bg-zinc-900/60`}>
            <div>
