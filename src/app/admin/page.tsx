...
import {
    ...
    Search, FileDown, UserX, Info, AlertCircle,
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm p-8 rounded-[3rem] shadow-2xl animate-in zoom-in duration-300">
                ...
                <h3 className="text-xl font-black italic uppercase text-center mb-2 text-white">
                    {title}
                </h3>
                <p className="text-zinc-500 text-[10px] font-bold text-center mb-8 uppercase tracking-widest leading-relaxed px-4">
                    {description}
                </p>
                ...
                <div className="flex flex-col gap-3">
                    <button
                        onClick={onConfirm}
                        disabled={loading}
                        className={`w-full py-4 rounded-2xl font-bold uppercase text-[10px] flex items-center justify-center gap-2 transition-all active:scale-95 ${btnColors[type as keyof typeof btnColors]}`}
                    >
                    ...
                    <button
                        onClick={onCancel}
                        disabled={loading}
                        className="w-full bg-zinc-800/50 py-4 rounded-2xl font-bold text-zinc-400 uppercase text-[10px] hover:bg-zinc-800 transition-all"
                    >
                        Cancelar
                    </button>
                </div>
    ...
    return (
        <div className={`bg-zinc-900/40 border ${alert ? 'border-red-500/20' : 'border-zinc-800/50'} p-6 rounded-[2.5rem] flex flex-col justify-between hover:bg-zinc-800/40 transition-all group relative overflow-hidden`}>
            ...
            <div className="flex items-center gap-1.5 mt-2 border-t border-zinc-800/50 pt-3">
                ...
                <p className={`text-[9px] font-bold uppercase tracking-tight ${alert ? "text-red-400" : "text-zinc-600"}`}>
                    {description}
                </p>
            </div>
        </div>
    ...
    const gerarPDF = () => {
        pedirConfirmacao(
            ...
            () => {
                ...
                const doc = new jsPDF();
                doc.setFontSize(20);
                doc.text("Relatorio Centro de Comando", 14, 20);
                autoTable(doc, {
                    startY: 30,
                    head: [['Metrica', 'Valor']],
                    body: [
                        ...
                        ['Taxa de Evasao', `${stats.churnRate}%`]
                    ],
                    theme: 'striped',
                    headStyles: { fillColor: [158, 205, 29] }
                });
                doc.save(`bxvs-relatorio-${new Date().toLocaleDateString()}.pdf`);
            }
        );
    };
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-8 pb-32 font-sans selection:bg-[#9ECD1D] selection:text-black">
            ...
            {showModalEncerrar && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                    <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md p-10 rounded-[3rem] shadow-2xl">
                        ...
                        <h2 className="text-3xl font-black italic uppercase text-center mb-2 leading-none">
                            Encerrar <span className="text-red-500">Dia?</span>
                        </h2>
                        <p className="text-zinc-500 text-xs font-bold text-center mb-10 uppercase tracking-widest">Todos os check-ins serão finalizados agora.</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={confirmarEncerrarDia} disabled={encerrando} className="w-full bg-red-600 hover:bg-red-500 py-5 rounded-2xl font-bold uppercase text-[10px] flex items-center justify-center gap-2 transition-all">
                                {encerrando ? <Loader2 className="animate-spin" size={16} /> : "Finalizar Expediente"}
                            </button>
                            <button onClick={() => setShowModalEncerrar(false)} className="w-full bg-zinc-800 py-5 rounded-2xl font-bold text-zinc-400 uppercase text-[10px]">Voltar ao Painel</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div>
                    <div className="flex items-center gap-3 mb-4">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-full">
                            ...
                            <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Unidade Live</span>
                        </div>
                    </div>
                    <h1 className="text-5xl font-black italic uppercase tracking-tighter">
                        CENTRO DE <span className="text-[#9ECD1D]">COMANDO</span>
                    </h1>
                </div>

                <div className="flex flex-wrap items-center gap-4">
                    <button onClick={gerarPDF} className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-6 py-3.5 rounded-2xl font-bold text-[10px] uppercase hover:bg-zinc-800 transition-all text-zinc-400">
                        <FileDown size={16} /> Exportar Relatório
                    </button>
                    <button onClick={() => setShowModalEncerrar(true)} className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 px-6 py-3.5 rounded-2xl font-bold text-[10px] uppercase text-red-500 hover:bg-red-500 hover:text-white transition-all">
                        <LogOut size={16} /> Encerrar Turno
                    </button>
                    <div className="relative">
                        <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600" size={18} />
                        <input
                            type="text" placeholder="BUSCAR ATLETA OU COMANDO..."
                            className="bg-zinc-900 border border-zinc-800 rounded-2xl py-3.5 pl-14 pr-6 text-[10px] font-bold focus:border-[#9ECD1D] outline-none w-64 transition-all focus:w-80 placeholder:text-zinc-800"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                ...
                <MetricCard title="Taxa de Evasão" value={`${stats.churnRate}%`} icon={<TrendingUp size={20} />} alert={Number(stats.churnRate) > 15} description="ÍNDICE DE DESISTÊNCIA" />
            </div>
                ...
                <div className="lg:col-span-2 space-y-10">
                    ...
                    <section className="bg-zinc-900/40 border border-zinc-800/50 p-8 rounded-[3rem]">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 text-zinc-400">
                            ...
                            <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-600">
                                <span className="w-2 h-2 bg-[#9ECD1D] rounded-full" /> TEMPO REAL
                            </div>
                        </div>
                    ...
                    <section>
                        <div className="flex items-center justify-between mb-8 px-4">
                            ...
                            <div className="flex items-center gap-4">
                                <p className="text-[10px] font-bold text-zinc-500">{presentes.length} ATLETAS</p>
                        ...
                        {presentes.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {presentes.filter(a => a.aluno_nome.toLowerCase().includes(termoPesquisa.toLowerCase())).map((aluno) => {
                                    ...
                                    return (
                                        <div key={aluno.email} className="bg-zinc-900/40 border border-zinc-800/50 p-5 rounded-[2.5rem] flex items-center justify-between group hover:border-[#9ECD1D]/20 transition-all">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-zinc-800 rounded-2xl flex items-center justify-center text-[#9ECD1D] font-black italic border border-white/5 uppercase">
                                                    {aluno.aluno_nome[0]}
                                                </div>
                                                <div>
                                                    <p className="text-sm font-black uppercase italic leading-none mb-1">{aluno.aluno_nome}</p>
                                                    <p className={`text-[9px] font-bold uppercase tracking-tight ${tempo > 90 ? 'text-red-500' : 'text-zinc-500'}`}>
                            ...
                            <div className="py-24 border-2 border-dashed border-zinc-900 rounded-[3rem] flex flex-col items-center justify-center text-zinc-800">
                                ...
                                <p className="text-[10px] font-bold uppercase tracking-[0.2em]">Nenhum atleta detectado</p>
                            </div>
                        )}
                    </section>
                </div>
                ...
                <div className="space-y-8">
                    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[3rem] shadow-xl">
                        <div className="flex justify-between items-end mb-6">
                            <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Lotação da Unidade</h3>
                            <span className="text-2xl font-black italic text-white">{lotacaoPercentual}%</span>
                        </div>
                    ...
                    <section className="bg-red-500/5 border border-red-500/10 p-8 rounded-[3rem]">
                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-red-500 mb-8 flex items-center gap-3">
                        ...
                        <div className="space-y-4">
                            {alunosEmRisco.map((aluno, i) => (
                                <div key={i} className="flex items-center justify-between bg-black/40 p-4 rounded-2xl group border border-transparent hover:border-red-500/20 transition-all">
                                    <div>
                                        <p className="text-xs font-black uppercase italic">{aluno.nome.split(' ')[0]}</p>
                                        <p className="text-[9px] font-bold text-zinc-600 uppercase">+7 DIAS AUSENTE</p>
                                    </div>
                                    <button
                                        onClick={() => {
                                            pedirConfirmacao(
                    ...
                    <section className="bg-zinc-900/40 border border-zinc-800/50 p-8 rounded-[3rem]">
                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-[#9ECD1D] mb-8 flex items-center gap-3">
                            <Bell size={18} /> Cobranças Pendentes
                        </h3>
                            ...
                            {alunosRecuperacao.slice(0, 4).map((aluno, i) => (
                                <div key={i} className="flex items-center justify-between border-b border-zinc-800/50 pb-4 last:border-0 last:pb-0">
                                    <div className="max-w-[140px]">
                                        <p className="text-xs font-black uppercase italic truncate">{aluno.nome}</p>
                                        <p className="text-[9px] text-red-500 font-bold uppercase">{aluno.status_assinatura}</p>
                                    </div>
                                    <button
                                        onClick={() => dispararCobranca(aluno)}
                                        className="group flex items-center gap-2 text-[9px] font-bold text-[#9ECD1D] uppercase tracking-widest hover:translate-x-1 transition-all"
                                    >
